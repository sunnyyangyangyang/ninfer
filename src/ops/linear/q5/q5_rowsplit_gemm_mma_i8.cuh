...
// Q5G64 RowSplit x INT8-MMA GEMM (int8 activation route).
//
// Q5 codes are five-bit values: a 4-bit low plane (32 packed bytes per
// group) plus a 1-bit high plane (8 bytes per group, one bit per code).
// The unsigned code c expands to the signed int8 value c - 16 in [-16, 15],
// which int8 represents exactly.
//
...
void q5_rowsplit_gemm_mma_i8_kernel(
...
#pragma unroll 1
        for (int item = tid; item < BN * x8; item += Schedule::kThreads) {
            ...
            auto* dst           = &Bs8[stage * (BN * BK) + local_col * BK + 2 * gemm_swz64(local_col, k16 * 8)];
            if constexpr (kFull) {
        ...
        for (int row_group = tid; row_group < BM * GPB; row_group += Schedule::kThreads) {
            ...
            if constexpr (kFull) {
                const std::int64_t group_index =
                    static_cast<std::int64_t>(row) * groups_per_row + group0 + group;
                cp_async<8>(
                    dst, &high[group_index * Q5RowSplitStorage::kHighBytesPerGroup]);
            } else {
                if (row < rows) {
                    const std::int64_t group_index =
                        static_cast<std::int64_t>(row) * groups_per_row + group0 + group;
    ...
    auto expand_weight = [&](int stage, int k_tile) {
        for (int local_row = warp; local_row < BM; local_row += Schedule::kWarps) {
            ...
            const std::uint8_t packed =
                Cr[stage][local_row * GPB * Q5RowSplitStorage::kCodeBytesPerGroup
                          + group * Q5RowSplitStorage::kCodeBytesPerGroup + lane];
            const std::uint8_t high_byte =
                Hr[stage][local_row * GPB * Q5RowSplitStorage::kHighBytesPerGroup
                          + group * Q5RowSplitStorage::kHighBytesPerGroup + (lane >> 2)];
            const int shift = (lane & 3) * 2;
            const int v0 = ((static_cast<int>(packed & 0x0fu) |
                             static_cast<int>(((high_byte >> shift) & 1) << 4)) - 16);
            const int v1 = ((static_cast<int>(packed >> 4) |
                             static_cast<int>(((high_byte >> (shift + 1)) & 1) << 4)) - 16);
            ...
            dst[gemm_swz64(local_row, group * (Q5RowSplitStorage::kGroupK / 2) + lane)] = pair;
        }
    };
...
#pragma unroll
            for (int mi = 0; mi < MT; ++mi) {
                ...
                ldmatrix_x4(a_frag[mi][0], a_frag[mi][1], a_frag[mi][2], a_frag[mi][3],
                            smem_addr(&Asu[row * KB2 + gemm_swz64(row, col)]));
            }
#pragma unroll
            for (int ni = 0; ni < NT; ++ni) {
                ...
                ldmatrix_x2(b_frag[ni][0], b_frag[ni][1],
                            smem_addr(&Bs8[stage * (BN * BK) + row * BK + 2 * gemm_swz64(row, col)]));
            }
        };
...
#pragma unroll
        ...
        {
            const int scale_off =
                (Schedule::kScaleLoadMode == Q5ScaleLoad::Pair32) ? ((k_tile & 1) * SB / 2) : 0;
            const int group = k_tile * GPB;
#pragma unroll
            for (int mi = 0; mi < MT; ++mi) {
            ...
            if constexpr (kFull) {
                out[static_cast<std::int64_t>(output_col0) * rows + output_row0] =
                    __float2bfloat16_rn(__bfloat162float(
                        out[static_cast<std::int64_t>(output_col0) * rows + output_row0]) + values[0]);
                out[static_cast<std::int64_t>(output_col1) * rows + output_row0] =
                    __float2bfloat16_rn(__bfloat162float(
                        out[static_cast<std::int64_t>(output_col1) * rows + output_row0]) + values[1]);
                out[static_cast<std::int64_t>(output_col0) * rows + output_row1] =
                    __float2bfloat16_rn(__bfloat162float(
                        out[static_cast<std::int64_t>(output_col0) * rows + output_row1]) + values[2]);
                out[static_cast<std::int64_t>(output_col1) * rows + output_row1] =
                    __float2bfloat16_rn(__bfloat162float(
                        out[static_cast<std::int64_t>(output_col1) * rows + output_row1]) + values[3]);
            } else {
                if (output_row0 < rows) {
                    if (output_col0 < cols) {
                        out[static_cast<std::int64_t>(output_col0) * rows + output_row0] =
